...
Chimera is Relay's cross-domain AI backbone. Rather than bolting isolated copilots onto individual tools, Chimera operates across the full software lifecycle — understanding code context, delivery history, test results, and production signals simultaneously.

## Why a unified engine matters
A code suggestion that ignores deployment constraints is incomplete. A deployment decision that ignores test coverage is risky. Chimera connects these domains so AI actions are grounded in the full picture:
...
## Architecture
Chimera combines retrieval-augmented generation with a structured knowledge graph. Every query is enriched with relevant context from the caller's domain and adjacent domains before the model generates a response.`,
      sections: [
    ...
    },
    {
      ...
      short: "A semantic index of code, configs, pipelines, incidents, and docs that gives every AI action rich, organisation-specific context.",
      d: `## AI Context Graph

The context graph is the retrieval backbone behind Chimera. It indexes an organisation's codebase, pipeline definitions, deployment history, incident records, runbooks, and internal documentation into a searchable semantic store.
...
The code agent operates inside the developer's workflow — IDE, pull request, or CLI — to accelerate coding tasks with organisation-aware intelligence.
...
- **Codebase Q&A** — answers questions about the codebase by searching the context graph for relevant code, docs, and historical changes.

## Delivery awareness
Unlike standalone copilots, the code agent knows how the code gets deployed. It can warn if a change will break a downstream pipeline, flag if the target service has an active incident, or note that the affected environment has a deployment freeze.`,
    },
    {
      id: "rai-agent-deploy",
      e: "🚀",
      c: WA,
      title: "Deploy Agent",
      short: "An intelligent deployment agent that plans rollout strategies, monitors canary health, and auto-remediates failures.",
      d: `## Deploy Agent

The deploy agent automates the judgment calls in the deployment process — choosing a rollout strategy, monitoring health signals, and deciding when to proceed or roll back.

## How it works
1. **Risk assessment** — analyses the change set (files modified, services affected, blast radius from the knowledge graph) and assigns a risk tier.
2. **Strategy selection** — picks the rollout strategy based on risk: low-risk changes deploy directly, medium-risk use canary with automated verification, high-risk require manual approval gates.
...
The agent improves its risk model over time by incorporating deployment outcomes. Services with a history of smooth deploys get faster rollouts; services that have caused incidents get additional scrutiny.`,
    },
    {
      id: "rai-agent-test",
      e: "🧪",
...
The test agent tackles the hardest parts of testing: writing meaningful tests, keeping them reliable, and running the right subset for each change.

## Capabilities
- **Test generation** — creates unit and integration tests from code analysis, guided by the context graph's understanding of the service's contracts and dependencies.
...
- **Flake diagnosis** — when a test is flagged as flaky, the agent analyses timing patterns, shared state, and external dependencies to suggest root-cause fixes.

## Impact on delivery speed
By running only relevant tests and quarantining known flakes, the test agent can dramatically reduce pipeline wait times while maintaining (or improving) the actual quality signal.`,
    },
    {
...
When an alert fires, the incident agent accelerates the path from detection to resolution by correlating production signals with delivery history.

## Investigation flow
1. **Alert enrichment** — when a monitor triggers, the agent gathers related metrics, logs, and traces for the affected service.
2. **Change correlation** — queries recent deployments, config changes, and infrastructure modifications from the knowledge graph to identify likely causes.
...
## Human-in-the-loop
The incident agent proposes actions but does not execute remediations autonomously unless explicitly configured. Rollback of a suspected bad deploy still requires human confirmation unless the team has opted into fully automated rollback policies.`,
    },
  ],
...
- Chimera maps this intent to concrete pipeline stages using the organisation's templates, artefact registries, and environment configurations from the knowledge graph.
...
3. **Root-cause attribution** — traces the anomaly back through the knowledge graph to the likely cause: a specific deployment, a config change, or an infrastructure scaling event.
...
Anomalies are surfaced in the Relay Cloud Costs dashboard alongside recommendations (right-size, switch to spot, delete idle resources) so teams can act immediately.`,
    },
  ],
};
