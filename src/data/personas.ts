...
- Talent: retain senior engineers by reducing toil and improving the inner loop
...
- **Relay Deploy** — progressive delivery, policy gates, automated verification
...
Build TCO live: pipeline maintenance FTE + incident cost + idle cloud waste. Their numbers, not yours.`},
    ...
    {title:"Relevant Relay modules",open:false,content:`- **Relay CI** + **Relay Deploy** — core delivery spine
- **Relay Verify** — post-deploy health checks and automated rollback paths
...
Ask calendar time and what they would *not* build while doing it. Usually 6–12 months of developer-facing work deferred.`},
    {title:"Relevant SDLC stages",open:false,content:`**Plan** (catalog/templates), **Build**, **Secure**, **Deploy**, **Observe**, **Optimise**. Platform leaders live across the outer loop; they care that Code-stage AI tools don't bypass their standards.`},
    {title:"Relevant Relay modules",open:false,content:`- **Relay Portal** — service catalog, golden paths, scorecards
- **Relay CI** / **Relay Deploy** — standardised pipelines and environments
- **Relay Secure** — org-wide policy and scanner orchestration
- **Relay IaC** — infra provisioning with guardrails
...
- **Relay Flags** — progressive exposure without separate vendors`},
  ...
  {id:"devops",e:"⚙️",c:WA,title:"Director of DevOps",role:"Technical Influencer / Co-Champion",
   ...
   sa:"Anchor on the last painful release: 'Walk me through the last bad deploy — detection, people on the bridge, rollback.' Then map Relay Deploy + Verify to those exact steps.",
...
Keep it. Relay Deploy + Verify as the CD and intelligence layer is a common entry.`},
    ...
    {title:"Relevant Relay modules",open:false,content:`- **Relay Deploy** — orchestration, strategies, approvals as code
- **Relay Verify** — metric/log-based post-deploy checks
- **Relay CI** — if CI maintenance or test intelligence is the wedge
- **Relay Secure** — scanners and gates in the same pipeline
- **Relay Observe** — change intelligence tied to incidents`},
  ]},
...
- **Relay Deploy** — cannot-bypass path for production
- **Relay Supply** — SBOM, provenance, dependency evidence
- **Relay Observe** — runtime signals tied to change
- **Relay Code** — secure defaults and policy hints in the AI coding loop`},
  ]},
...
- Team satisfaction with local inner loop and CI feedback time`},
...
- Show how Relay Plan + Code + Deploy reduces tool hopping
...
Golden path must be the easiest path — if Relay is harder than snowflake scripts, they will route around it.`},
    {title:"Relevant SDLC stages",open:false,content:`**Plan** (work clarity), **Code** (inner loop), **Build** (fast signal), **Deploy** (safe release), **Observe** (own their services).`},
    {title:"Relevant Relay modules",open:false,content:`- **Relay Plan** — work linked to pipelines and releases
- **Relay Code** — AI assist inside standards
- **Relay CI** — faster, smarter feedback
- **Relay Deploy** / **Relay Verify** — less babysitting
...
- **Relay Plan** — work-item linkage and release visibility
- **Relay Code** — standards-aware AI assistance
- **Relay CI** / **Relay Deploy** — paved-road execution
...
- **Relay Deploy** / **Relay IaC** — environment lifecycle controls
